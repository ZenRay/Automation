...
,cat4_stat AS(
    SELECT
        ...
        ,SUM(t1.deliveried_gross_wgt) AS deliveried_gross_wgt -- `送达重量`
        ,SUM(t1.is_filtered * t1.delivered_goods_amt) AS delivered_goods_amt_filtered -- `非榴莲品类送达金额`
        ,SUM(t1.final_refund_amt) AS final_refund_amt -- `赔付金额`
        ,SUM(t1.is_filtered * t1.final_refund_amt) AS final_refund_amt_filtered -- `非榴莲品类赔付金额`
        ...
        ,SUM(t1.is_vegitable * t1.deliveried_gross_wgt) AS deliveried_gross_wgt_vegitable -- `蔬菜送达重量`
        ...
        ,SUM(t1.is_fruit * t1.deliveried_gross_wgt) AS deliveried_gross_wgt_fruit -- `水果送达重量`
...
SELECT
    ...
    ,NVL(t1.deliveried_gross_wgt,0) AS `送达重量`
    ,NVL(t1.delivered_goods_amt_vegitable,0) AS `蔬菜送达金额`
    ,NVL(t1.deliveried_gross_wgt_vegitable,0) AS `蔬菜送达重量`
    ,NVL(t1.delivered_goods_amt_fruit,0) AS `水果送达金额`
    ,NVL(t1.deliveried_gross_wgt_fruit,0) AS `水果送达重量`
	,NVL(t1.final_refund_amt,0)  `赔付金额`
    ,ROUND(
        NVL(t1.final_refund_amt / t1.delivered_goods_amt, 0), 5
    ) AS `售后赔付率`
    ,NVL(t1.delivered_goods_amt_filtered,0)  `非榴莲品类送达金额`
	,NVL(t1.final_refund_amt_filtered,0)  `非榴莲品类赔付金额`
    ,ROUND(
