...
#   --help, -h             显示帮助信息
#
#   任务名称：okr | daily_report | upgrade_after_sale（缩写 ua / dr）
#
# 任务控制示例：
#   ./cron_task.sh --skip-task okr                 # 跳过 OKR，运行 Daily Report + Upgrade
#   ./cron_task.sh --only-task okr --only-task ua  # 仅运行 OKR + Upgrade（ua=upgrade_after_sale）
#   ./cron_task.sh 2026-07-21 --skip-task okr       # 指定日期 + 跳过 OKR
#
...
示例:
  ...
  cron_task.sh --skip-task okr                  # 跳过 OKR
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --skip-task)
            if [ -z "${2:-}" ]; then
                echo "错误: --skip-task 需要参数 (okr | daily_report | upgrade_after_sale)" >&2
                exit 1
            fi
        ...
        --only-task)
            if [ -z "${2:-}" ]; then
                echo "错误: --only-task 需要参数 (okr | daily_report | upgrade_after_sale)" >&2
                exit 1
            fi
...
if python -m workers.okr.main "${ARGS[@]}" 2>&1 | tee -a "$CRON_LOG_FILE"; then
    ...
    EXIT_CODE=$?
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [Task 1/3] OKR 数据管道 - FAILED (exit_code=$EXIT_CODE), continuing..." | tee -a "$CRON_LOG_FILE"
    TASKS_RUN=$((TASKS_RUN + 1))
...
if ! _task_enabled "daily_report"; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [Task 2/3] Daily Report 日报 - SKIPPED (--skip-task/--only-task)" | tee -a "$CRON_LOG_FILE"
    TASKS_SKIPPED=$((TASKS_SKIPPED + 1))
else

echo "[$(date '+%Y-%m-%d %H:%M:%S')] [Task 2/3] Daily Report 日报 - START (参数: ${ARGS[*]:-默认})" | tee -a "$CRON_LOG_FILE"

if python -m workers.daily_report.main "${ARGS[@]}" 2>&1 | tee -a "$CRON_LOG_FILE"; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [Task 2/3] Daily Report 日报 - SUCCESS" | tee -a "$CRON_LOG_FILE"
    TASKS_RUN=$((TASKS_RUN + 1))
else
    EXIT_CODE=$?
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [Task 2/3] Daily Report 日报 - FAILED (exit_code=$EXIT_CODE), continuing..." | tee -a "$CRON_LOG_FILE"
    TASKS_RUN=$((TASKS_RUN + 1))
    TASKS_FAILED=$((TASKS_FAILED + 1))
fi
