...
class OAuthCallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if "code" in query_components:
            ...
            logger.error(
                f"Authorization failed with error: {OAuthCallbackHandler.auth_error}"
            )
        else:
            self.send_response(400)
            self.end_headers()
...
def start_local_server(
    ...
    try:
        ...
        if e.errno == 48:  # Address already in use
            raise OSError(
                f"Port {port} is already in use. Please use a different port with --port option"
            ) from e
        raise
    ...
    if OAuthCallbackHandler.auth_code:
        ...
        raise Exception(
            f"OAuth authorization failed: {OAuthCallbackHandler.auth_error}"
        )
    else:
