...
class SpreadSheetMeta:
    ...
    def sheet_items(self, value):
        if isinstance(value, dict):
            ...
            isinstance(item, (dict, SheetItem)) for item in value
        ):
            for item in value:
                if isinstance(item, dict):
                    ...
                    raise LarkSheetException(
                        f"Invalid sheet item type: {type(item)}. Must be dict or SheetItem."
                    )
        else:
            raise LarkSheetException(
                f"Invalid sheet items type: {type(value)}. Must be dict, SheetItem, or list/tuple of dict/SheetItem."
            )
