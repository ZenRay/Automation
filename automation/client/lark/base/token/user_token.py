...
class UserAccessToken:
    ...
    def _get_connection(cls) -> sqlite3.Connection:
        ...
        conn.row_factory = (
            sqlite3.Row
        )  # Make query results accessible like dictionaries
        try:
    ...
    def _ensure_table_exists(cls):
        ...
        with cls._get_connection() as conn:
            ...
            if not conn.execute(sql).fetchone():
                ...
                # Create indexes
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_user_id ON user_access_tokens(user_id)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_open_id ON user_access_tokens(open_id)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_union_id ON user_access_tokens(union_id)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_user_email ON user_access_tokens(user_email)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_status ON user_access_tokens(status)"
                )
                conn.commit()
                ...
                try:
                    ...
                    with self._get_connection() as conn:
                        if self.id is None:
                            ...
                            if self.open_id and self.tenant_key:
                                row = conn.execute(
                                    "SELECT id FROM user_access_tokens WHERE open_id = ? AND tenant_key = ?",
                                    (self.open_id, self.tenant_key),
                                ).fetchone()
                            elif self.user_id:
                                row = conn.execute(
                                    "SELECT id FROM user_access_tokens WHERE user_id = ?",
                                    (self.user_id,),
                                ).fetchone()
                            else:
                                row = None
                        ...
                        conn.commit()
                        logger.info(
                            (msg and f"Execute Task<{msg}> .")
                            + f"Token saved successfully for user {self.user_id}"
                        )
                        return True
                    ...
                    if "locked" in msg or "database is locked" in msg:
                        if attempt < attempts:
                            ...
                            logger.error(
                                (msg and f"Execute Task<{msg}> .")
                                + f"Token save failed. Database locked, max retries reached: {e}"
                            )
                            return False
                    else:
                        logger.error(
                            (msg and f"Execute Task<{msg}> .")
                            + f"Operational error saving token: {e}"
                        )
                        return False
                except sqlite3.IntegrityError as e:
                    logger.error(
                        (msg and f"Execute Task<{msg}> .")
                        + f"Token save failed. Database integrity error: {e}"
                    )
                    ...
                    logger.error(
                        (msg and f"Execute Task<{msg}> .")
                        + f"Token save failed. Error saving token: {e}"
                    )
                    return False
            ...
            with cls._get_connection() as conn:
                ...
                for key, value in conditions.items():
                    if key in [
                        ...
                        params.append(
                            value.value if isinstance(value, TokenStatus) else value
                        )
    ...
    def auto_refresh(self, client) -> None:
        ...
        if (
            getattr(self, "refresh_token_expires_at", None)
            and self.refresh_token_expires_at
            and datetime.now() >= self.refresh_token_expires_at
        ):
            logger.error(
                f"Refresh token expired for user {self.user_id}; cannot auto-refresh"
            )
        ...
        with self._lock:
            try:
                ...
                logger.debug(
                    f"Start auto refreshing token for user<id: {self.user_id} name: {self.user_name}>"
                )
                ...
                if new_token_data:
                    self.access_token = getattr(
                        new_token_data, "access_token", self.access_token
                    )
                    ...
                    if hasattr(new_token_data, "expires_in"):
                        self.expire_time = start_time + timedelta(
                            seconds=getattr(new_token_data, "expires_in", 0)
                        )
                    if hasattr(new_token_data, "refresh_expires_in"):
                        self.refresh_token_expires_at = start_time + timedelta(
                            seconds=getattr(new_token_data, "refresh_expires_in", 0)
                        )

                    if hasattr(new_token_data, "scope"):
                        self.scopes = (
                            getattr(new_token_data, "scope", "").split(" ")
                            or self.scopes
                        )
                    self.status = TokenStatus.ACTIVE
                    logger.info(
                        f"Token refreshed successfully for user<id: {self.user_id} name: {self.user_name}>"
                    )
                    self.save(msg="Transform Status to ACTIVE after refresh")
                else:
                    logger.error(
                        f"Token refresh returned no data for user<id: {self.user_id} name: {self.user_name}>"
                    )
                    self.status = TokenStatus.INVALID
                ...
                logger.error(
                    f"Refreshing token failed for user<id: {self.user_id} name: {self.user_name}> failed: {e}"
                )
                self.status = TokenStatus.INVALID
                self.save(
                    msg="Transform Status to INVALID after exception during refresh"
                )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"UserAccessToken(id={self.id}, open_id={self.open_id}, user_id={self.user_id}, "
    ...
    def from_oauth_response(
        ...
        source: str = "oauth_authorization",
    ) -> "UserAccessToken":
        ...
        scopes = (
            scopes_raw.split() if isinstance(scopes_raw, str) and scopes_raw else []
        )
        ...
        try:
            refresh_expires_in = (
                oauth_json.get("refresh_expires_in")
                or oauth_json.get("refresh_expires")
                or oauth_json.get("refresh_token_expires_in")
                or oauth_json.get("refresh_token_expires")
            )
    ...
    def exchange_code_for_token(
        ...
        need_fetch_user_info: bool = True,
    ) -> "UserAccessToken":
        ...
        try:
            ...
            raise LarkException(
                "App credentials required: provide a `client` with `app_id` and `app_secret`"
            )
        ...
        if need_fetch_user_info and access_token:
            if not (
                hasattr(client, "query_user_info")
                and callable(getattr(client, "query_user_info"))
            ):
                raise LarkException(
                    "Client does not support fetching user info by token"
                )
        ...
        token = cls.from_oauth_response(
            oauth_payload, user_info=user_info, source="oauth_authorization"
        )
        token.save()
        return token

    @classmethod
    def extract_token_by_username(
        cls, client, user_name: str, force_refresh: bool = False
    ) -> None:
        ...
        if not token.is_valid and token.needs_refresh():
            logger.info(
                f"Token for user_name={user_name} needs refresh; attempting auto-refresh"
            )
            token.auto_refresh(client=client)
    ...
    def refresh(
        self, client: Optional[object], user_info_fetcher: Optional[Callable] = None
    ) -> "UserAccessToken":
        ...
        with self._lock:
            ...
            try:
                ...
                if result.get("code") != 0 and not result.get("access_token"):
                    error_msg = (
                        result.get("error_description") or result.get("msg") or result
                    )
                    self.status = TokenStatus.INVALID
                    self.save(msg="Transform Status to INVALID after failed refresh")
                ...
                refresh_expires_in = (
                    ...
                    or result.get("refresh_expires")
                    or result.get("refresh_token_expires_in")
                    or result.get("refresh_token_expires")
                )
                if refresh_expires_in:
                    try:
                        self.refresh_token_expires_at = start_time + timedelta(
                ...
                logger.info(
                    "Authorization scopes: %s",
                    " ".join(self.scopes) or "（无/使用应用默认）",
                )
                ...
                if user_info_fetcher and self.access_token:
                    try:
                        ...
                        if user_info:
                            ...
                            self.tenant_key = (
                                user_info.get("tenant_key") or self.tenant_key
                            )
                    except Exception:
                        pass
