    ...
    def check_validate(self, *args, **kwargs):
        ...
        if not isinstance(self.text, str) or len(self.text) == 0:
            raise LarkMessageException(
                "Text message content must be a non-empty string."
            )
        return True
...
class ImageMessage(Message):
    ...
    def check_validate(self, file: str):
        ...
        file_types = ("jpg", "jpeg", "png", "webp", "gif", "bmp", "ico", "tiff", "heic")
        _, extension = self._extract_file_info(file)

        if extension not in file_types:
            raise LarkMessageException(
                "Invalid image file type, must be one of: {}".format(
                    ", ".join(file_types)
                )
            )
        return True
    ...
    def upload_file(
        self, func, file: str = None, image_type: str = "message", *args, **kwargs
    ):
        """Upload File With API
        ...
        if not check_function_arg(func, "need_binary"):
            raise TypeError(
                "uploader 'func' must accept a 'need_binary' keyword argument"
            )
        ...
        if self.is_can_upload:
            result = func(
                file=file, need_binary=True, image_type=image_type, *args, **kwargs
            )
            logger.debug(
                f"Image uploaded via uploader callable; local_file={self.file}"
            )
    ...
    @staticmethod
    def static_upload_image2key(
        func, file: str, image_type: str = "message", *args, **kwargs
    ):
    ...
    def send_message(
        ...
        **kwargs,
        ...
        content = {"image_key": self.image_key}
...
class FileMessage(Message):
    ...
    SPECIFIC_FILE_TYPES = ("opus", "mp4", "pdf", "doc", "xls", "ppt")
    AUDIO_MESSAGE_TYPES = ("opus",)
    MEDIA_MESSAGE_TYPES = ("mp4",)
    ...
    def __init__(self, file: str = None):
        ...
        if file is not None and self.check_validate(file):
            ...
            if self._file_type in self.SPECIFIC_FILE_TYPES:
                ...
                logger.warning(
                    f"File type '{self._file_extension}' is not a specific type, defaulting to 'stream'."
                )
    ...
    def file_type(self, value):
        ...
        if isinstance(value, str):
            if value.lower() in self.SPECIFIC_FILE_TYPES:
                ...
                logger.warning(
                    f"File type '{value}' is not a specific type, defaulting to 'stream'."
                )
        else:
            raise LarkMessageException("File type must be a string.")
    ...
    def msg_type(self, value):
        ...
        if value.lower() in self.AUDIO_MESSAGE_TYPES:
            ...
            raise LarkMessageException(
                "Message type must be one of: stream, {}".format(
                    ", ".join(self.SPECIFIC_FILE_TYPES)
                )
            )
    ...
    def media_coverage_key(self, value):
        ...
        if self.msg_type not in self.MEDIA_MESSAGE_TYPES:
            raise LarkMessageException(
                "Media coverage key can only be set for media message type."
            )

        if isinstance(value, ImageMessage):
            if value.image_key is None:
                raise LarkMessageException(
                    "Media coverage ImageMessage must have a valid image_key."
                )
            ...
            raise LarkMessageException(
                "Media coverage key must be a image key string or ImageMessage instance."
            )
    ...
    def upload_file(self, func, *args, **kwargs):
        ...
        if not check_function_arg(func, "file_type"):
            raise TypeError(
                "uploader 'func' must accept a 'file_type' keyword argument"
            )

        if not check_function_arg(func, "file_name"):
            raise TypeError(
                "uploader 'func' must accept a 'file_name' keyword argument"
            )
        ...
        if self.is_can_upload:
            ...
            if isinstance(result, dict) and result.get("code", -1) == 0:
                ...
                logger.info(
                    f"File ({self._file_name}) key updated to: {self._file_key}"
                )
            return result
        else:
        ...
        **kwargs,
        ...
        if file_key is not None:
            ...
            if msg_type is not None:
                ...
                raise LarkMessageException(
                    "When providing 'file_key' via kwargs, 'msg_type' must also be provided."
                )
        elif self.msg_type in ("audio", "file"):
            content = {"file_key": self.file_key}

        elif self.msg_type in ("media",):
            content = {"file_key": self.file_key}
        else:
            raise LarkMessageException(
                "Invalid message type for file message, or missing file_key."
            )
        ...
        if need_covarage_file and msg_type in ("media",):
            if use_args and kwargs.get("image_key", None) is not None:
                ...
                raise LarkMessageException(
                    "Media message coverage file required but no image_key provided."
                )
...
class StaticInteractiveMessage(Message):
    ...
    def msg_type(self, value):
        """Set Message Type Property"""
        raise NotImplementedError(
            "InteraciiveMessage msg_type is fixed to 'interactive'."
        )
    ...
    def check_validate(self, *args, **kwargs):
        ...
        if not isinstance(self._card, dict) or len(self._card) == 0:
            raise LarkMessageException(
                "Interactive message card content must be a non-empty dict."
            )
        return True
