...
class SendContentItem(ContentItem):
    ...
    def content(self, value):
        ...
        # Interactive Card Content
            ...
            isinstance(value, dict)
            and (value.get("type", "") in ("card", "template") or "schema" in value)
        ...
        # Share Chat Content
            ...
            isinstance(value, dict)
            and "chat_id" in value
            and self.msg_type == "share_chat"
        ):
            self._content = value
        # Share User Content
        elif (
            ...
            and self.msg_type == "share_user"
        ):
            self._content = value
        # Audio, media, file, sticker Content
        elif (
            isinstance(value, dict)
            and "file_key" in value
            and self.msg_type in ("audio", "media", "file", "sticker")
        ):
            self._content = value
        elif (
            isinstance(value, dict)
            and value.get("type", "") == "driver"
            and self.msg_type in ("system",)
        ):
            self._content = value
            ...
            raise ValueError(
                "Invalid content format, please check the msg_type and content structure."
            )
    ...
    def to_dict(self):
        ...
        base_dict["content"] = json.dumps(
            self.content if self.content else {}, ensure_ascii=False
        )
        return base_dict
    ...
    def send_message(self, func, receive_id_type, *, content, **kwargs):
        ...
        if receive_id_type not in self.RECEIVER_ID_TYPES:
            raise ValueError(
                f"Invalid receive_id_type. Must be one of {self.RECEIVER_ID_TYPES}."
            )
            ...
            raise ValueError(
                f"Receiver does not have a valid ID for receive_id_type '{receive_id_type}'."
            )
