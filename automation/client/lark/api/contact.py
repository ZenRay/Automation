...
class LarkContact(LarkClient):
    ...
    def __init__(
        ...
        *,
        app_id,
        app_secret,
        master_info={"user_name": "", "user_email": ""},
        ...
        if len(users) != 1:
            logger.warning(
                f"Master user not found or multiple found for info: {master_info}. User access token functions may not work properly."
            )
            self.__master_user_access_token = None
        else:
            self.__master_user_access_token = users[0]
            logger.info(
                f"Master user access token loaded: {self.__master_user_access_token}"
            )
    ...
    def user_access_token(self) -> str:
        ...
        if self.__user_access_token.needs_refresh():
            logger.info(
                f"Token for user {self.__user_access_token.user_id} needs refresh"
            )
        ...
        if self.__user_access_token.is_valid:
            ...
            logger.warning(
                f"User access token is not valid (status: {self.__user_access_token.status.value})"
            )
            return None
    ...
    def _query_user_info(cls, user_access_token: str) -> dict:
        ...
        response = lark_request.request(method="GET", url=url, headers=headers)
        ...
        logger.info(
            f"Successfully retrieved user info for user_id: {user_info.get('user_id')}"
        )
        return user_info

    def search_user(
        self, keyword: str, page_size: int = 10, page_token: str = None
    ) -> dict:
