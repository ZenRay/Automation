...
class LarkSheets(LarkClient):
    ...
    def __init__(
        self, app_id, app_secret, lark_host="https://open.feishu.cn", url=None
    ):
        ...
        self._regex_pattern = re.compile(
            r"http.*/(?P<app_type>sheets|wiki)/(?P<token>[a-zA-Z0-9_-]+)(?:\?sheet=(?P<sheet_id>[a-zA-Z0-9_-]+))?",
            re.I,
        )
    ...
    def spread_sheet(self, value):
        ...
        logger.info(
            f"Update Spread Sheet Token: {self._spread_sheet.spreadsheet_token} Success."
        )
    ...
    def extract_spreadsheet_info(self, url: str):
        ...
        if self._app_type == "sheets":
            ...
            if resp.get("code", -1) == 0:
                ...
                if obj_token:
                    ...
                    logger.info(
                        f"Retrieved spreadsheet token {obj_token} from wiki node"
                    )
                else:
                    logger.error(
                        f"Failed to extract spreadsheet token from wiki node: {resp}"
                    )
                    raise LarkException(
                        msg="Could not extract spreadsheet token from wiki node"
                    )
            else:
                logger.error(f"Failed to get wiki node information: {resp}")
                raise LarkException(
                    code=resp.get("code", -1),
                    msg=resp.get("msg", "Failed to get wiki node information"),
                )
        ...
        logger.info(
            f"Extracted spreadsheet token: {self._spread_sheet.spreadsheet_token}, sheet id: {self._active_sheet_id}, app type: {self._app_type}"
        )
        ...
        if resp.get("code", -1) == 0:
            ...
            for sheet in sheets:
                self._spread_sheet.sheet_items = SheetItem(
                    sheet_id=sheet.get("sheetId"), sheet_title=sheet.get("title")
                )
            return meta_data
        else:
            ...
            raise LarkException(
                code=resp.get("code"),
                msg=resp.get("msg", "Get spreadsheet metadata failed"),
            )
    ...
    def read_sheet_values(
        ...
        url = (
            f"{self._host}/open-apis/sheets/v2/spreadsheets/{token}/values/{range_str}"
        )
        ...
        if resp.get("code", -1) == 0:
            ...
            raise LarkException(
                code=resp.get("code"), msg=resp.get("msg", "Read sheet values failed")
            )
    ...
    def update_sheet_values(
        ...
        if resp.get("code", -1) == 0:
            ...
            raise LarkException(
                code=resp.get("code"), msg=resp.get("msg", "Update sheet values failed")
            )
    ...
    def append_sheet_values(
        ...
        if resp.get("code", -1) == 0:
            ...
            raise LarkException(
                code=resp.get("code"), msg=resp.get("msg", "Append sheet values failed")
            )

    def batchupdate_values_single_sheet(
        self,
        datas: List[List],
        data_range: str,
        spreadsheet_token: str = None,
        *,
        ...
        # Prepare Request
        url = (
            f"{self._host}/open-apis/sheets/v2/spreadsheets/{token}/values_batch_update"
        )
        headers = {
            "Content-Type": "application/json; charset=utf-8",
            "Authorization": f"Bearer {self.access_token}",
        ...
        if isinstance(data_range, str):
            ...
            for index, item in enumerate(
                data_generator(datas, self._UPDATE_ROW_LIMITATION)
            ):
                # adjust cell
                ...
                payload = {
                    "valueRanges": [
                        {"range": f"{sheet_id}!{data_range}", "values": item}
                    ],
                    "valueInputOption": value_input_option,
                }

                resp = request(
                    "POST", url, headers, payload=payload, refresh_client=self
                )

                if resp.get("code", -1) == 0:
                    logger.debug(
                        f"Batch update sheet values success for range: {data_range}"
                    )
                    time.sleep(2)
                else:
                    ...
                    raise LarkException(
                        code=resp.get("code"),
                        msg=resp.get("msg", "Batch update sheet values failed"),
                    )
    ...
    def add_sheet(self, properties: dict, spreadsheet_token: str = None):
        ...
        url = (
            f"{self._host}/open-apis/sheets/v2/spreadsheets/{token}/sheets_batch_update"
        )
        headers = {
            "Content-Type": "application/json; charset=utf-8",
            "Authorization": f"Bearer {self.access_token}",
        ...
        if resp.get("code", -1) == 0:
            ...
            raise LarkException(
                code=resp.get("code"), msg=resp.get("msg", "Add sheet failed")
            )
    ...
    def delete_sheet(self, sheet_id: str, spreadsheet_token: str = None):
        ...
        if self._check_sheet_exits(sheet_id):
            ...
            if resp.get("code", -1) == 0:
                ...
                logger.error(
                    f"'{sheet_id}'('{raw_sheet_id}') exists, but delete sheet failed: {resp}"
                )
                raise LarkException(
                    code=resp.get("code"), msg=resp.get("msg", "Delete sheet failed")
                )
        else:
            logger.error(f"Sheet with ID '{raw_sheet_id}' does not exist")
            raise LarkSheetException(
                msg=f"Sheet with ID '{raw_sheet_id}' does not exist"
            )

    def clear_sheet_values(
        self, range_str, spreadsheet_token: str = None, sheet_id: str = None
    ):
            ...
            try:
                ...
                if ":" in range_str:
                    ...
                    if not re.search(r"\d", cell):
                        current_col_data = (
                            self.read_sheet_values(f"{sheet_id}!{cell}")
                            .get("valueRange", {})
                            .get("values", [])
                        )
                        time.sleep(2)
                ...
                if total_row_records < self._UPDATE_ROW_LIMITATION:
                    less_row_limitation_ranges.append(
                        {"range": range_str, "values": empty_values}
                    )
                else:
                ...
                raise LarkException(
                    msg=f"Failed to process range {range_str}: {str(e)}"
                )
        ...
        if len(over_row_limitation_ranges) > 0:
            ...
            for range_info in over_row_limitation_ranges:
                self.batchupdate_values_single_sheet(
                    range_info["values"],
                    data_range=range_info["range"],
                    sheet_id=sheet_id,
                )

                time.sleep(2)

    def update_single_range_style(
        self,
        range_str: str,
        format_style: dict,
        spreadsheet_token: str = None,
        ...
        range_str = self._update_data_range(range_str, sheet_id=sheet_id)
        payload = {"appendStyle": {"range": range_str, "style": format_style}}
    ...
    def _update_data_range(
        self, range_str: str, sheet_id: str = None, update_sheet_method: str = "old"
    ):
        """Update Data Range
    ...
    def extract_sheets(self, spreadsheet_token: str = None):
        ...
        if resp.get("code", -1) == 0:
            ...
            raise LarkException(
                code=resp.get("code"), msg=resp.get("msg", "Get sheets list failed")
            )
    ...
    def _check_sheet_exits(self, sheet_id: str):
        ...
        sheet = self._spread_sheet.get_sheet_by_id(
            sheet_id
        ) or self._spread_sheet.get_sheet_by_title(sheet_id)
