...
class AdjustValue:
    ...
    def setup_value_by_fixed_value(cls, fixed_value: float) -> float:
        ...
        try:
            ...
            raise ValueError(
                f"Error validating commission rates by setup fixed commission rate: {e}"
            )
        return fixed_value
    ...
    @classmethod
    def offset_range_value(
        cls,
        value: float,
        low_threshold: float,
        high_threshold: float,
        keep_over_high_threshold: bool = False,
    ) -> float:
        ...
        if keep_over_high_threshold and value >= high_threshold:
            ...
            adjust_value = high_threshold + (
                high_threshold - low_threshold
            ) / high_threshold * (value - high_threshold)

        return cls._check_value_validate(adjust_value)

    @classmethod
    def setup_by_ladder_config(
        cls,
        condition_value: float,
        ladder_condition_config: list[float],
        ladder_value_config: list[float],
    ) -> float:
        ...
        if len(ladder_condition_config) != len(ladder_value_config):
            raise ValueError(
                "Ladder condition and value configurations must have the same length."
            )
        ...
        for i in range(len(ladder_condition_config)):
            if condition_value <= ladder_condition_config[i]:
                try:
                    ...
                    logger.error(f"Error validating commission rates: {e}")
                    raise ValueError(
                        f"Error validating commission rates by setup ladder commission rate: {e}"
                    )
                break
