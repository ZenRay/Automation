...
class Maxcompute2LarkOperator(BaseOperator):
    ...
    @apply_defaults
    def __init__(self, maxcompute_conn_id, lark_conn_id, *args, **kwargs):
        """
    ...
    def execute(self, context):
        ...
        # Maxcompute ETL
        instance = self._maxcompute_etl(sql=sql, context=context)
        ...
        if client_type == "multi":
            ...
            self._update_lark_multi_dimension_table(
                context=context, url=url, instance=instance
            )
        ...
        if message is not None:
            self._send_lark_message(context=context, message=message)
    ...
    def _maxcompute_etl(self, sql, context, **kwargs):
        ...
        if not self.maxcompute_hook:
            self.maxcompute_hook = MaxcomputeHook(conn_id=self.maxcompute_conn_id)
        ...
        instance = client.execute_sql(
            sql, hints=hints, is_async=False, interactive=False
        )
    ...
    def _update_lark_multi_dimension_table(
        self, context, url: str, instance=None, **kwargs
    ):
        """Update Lark Multi Dimension Table
        ...
        if is_clear:
            ...
            if filter is None:
                ...
                request_records = client.request_records_generator(
                    url=url, filter=filter, table_id=table_id
                )
            else:
            ...
            for start, end in zip(index, index[1:] + [len(records_id_list)]):
                client.delete_batch_records(
                    url=url, table_id=table_id, records_id=records_id_list[start:end]
                )
                time.sleep(2)
        ...
        for batch in instance.iter_pandas(batch_size=client.ADD_RECORD_LIMITATION):
            ...
            for start, end in zip(index, index[1:] + [batch.shape[0]]):
                ...
                client.add_batch_records(
                    records=records, url=url, table_id=table_id, table_name=table_name
                )
                time.sleep(2)
    ...
    def _update_lark_apaas_service_workspaces(self, context, instance=None, **kwargs):
        ...
        if filter_conditions is not None and len(filter_conditions) > 0:
            ...
            logger.info(
                f"Existing Records Deleted from aPaaS Service Workspace Table{workspace_id}:{table_name} with Filter Conditions: {filter_conditions}".format(
                    workspace_id=workspace_id,
                    table_name=table_name,
                    filter_conditions=filter_conditions,
                )
        ...
        if instance is not None:
                    ...
                    if batch[col].dtype == "object":
                        ...
                        try:
                            if (
                                ...
                                .apply(
                                    lambda x: (
                                        isinstance(x, Decimal) if pd.notna(x) else False
                                    )
                                )
                                .any()
                            ):
            ...
            logger.info(
                f"Maxcompute ETL Records Send to aPaaS Service Workspace Table {workspace_id}:{table_name} Success".format(
