#coding:utf-8
"""
Maxcompute Dimention Store Table ETL Sentence
1. 门店历史交易统计临时表 dim_store_history_trade_temp_sentence:  t_changsha_store_trade_t0d 用于支撑创建门店维度表的扩展信息
2. 门店维度表 dim_store_sentence: dim_changsha_store_daily_full 该门店表主要关注的是长沙仓的有真实意义的门店，因此其他仓和员工测试店铺不是我们关注的重点
"""
...
,customer AS(
    ...
    FROM(
        ...
        LEFT JOIN datawarehouse_max.ods_bg_users_demeter_users_full t2
            ON t2.pt BETWEEN DATE_FORMAT(DATEADD(CURRENT_DATE(), -3, "dd"), "yyyyMMdd000000")
