...
验证 sku_stat 路由的 商品id + 日期 复合 row_key 构建逻辑。
"""
...
def test_build_row_key_sku_stat_composite():
    """sku_stat_detail 使用 商品id + 日期 复合键"""
    df = pd.DataFrame(
        {
            "商品id": [5001, 5002, 5001],
    ...
    assert row_key.iloc[0] == "5001_2026-06-20"
    assert row_key.iloc[1] == "5002_2026-06-20"
    assert row_key.iloc[2] == "5001_2026-06-21"
...
def test_inject_row_key_sku_stat():
    ...
    assert mc_data["sku_stat"]["row_key"].iloc[0] == "5001_2026-06-20"
