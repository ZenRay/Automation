...
class TestStage2FilterProducts:
    def test_inner_join_keeps_matching(self, mock_conf_goods, mock_conf_trial_goods):
        result = filter_trial_products(mock_conf_goods, mock_conf_trial_goods, TARGET_DATE)
        assert len(result) == 2
    ...
    def test_commission_rate_from_trial_goods(self, mock_conf_goods, mock_conf_trial_goods):
...
class TestStage3MarkRegions:
    def test_marks_trial_and_non_trial(self, mock_conf_county, mock_conf_trial_group):
        result = mark_trial_regions(mock_conf_county, mock_conf_trial_group, TARGET_DATE)
        assert len(result) == 5
        trial = result[result["是否试验区域"] == 1]
        assert len(trial) == 2
        assert set(trial["市id"]) == {10, 20}

    def test_non_trial_has_empty_trial_fields(self, mock_conf_county, mock_conf_trial_group):
        result = mark_trial_regions(mock_conf_county, mock_conf_trial_group, TARGET_DATE)
...
class TestStage4AssociateCommission:
    def test_trial_gets_commission(self, mock_conf_county, mock_conf_trial_group,
                                   mock_conf_trial_commission):
        regions = mark_trial_regions(mock_conf_county, mock_conf_trial_group, TARGET_DATE)
        result = associate_commission(regions, mock_conf_trial_commission, TARGET_DATE)
        trial = result[result["是否试验区域"] == 1]
        assert (trial["抽佣率"] > 0).all()

    def test_non_trial_has_nan_commission(self, mock_conf_county, mock_conf_trial_group,
                                            mock_conf_trial_commission):
...
class TestStage5LogisticsFee:
    def test_no_match_returns_nan(self):
        ...
        regions = pd.DataFrame({
            "日期": [TARGET_DATE], "试验区域id": [1001], "试验区域名称": ["A"],
            "市id": [999], "省id": [1], "区县id": [100],
            "是否试验区域": [1], "抽佣率": [0.03],
        })
        logistics = pd.DataFrame({
            "日期": [TARGET_DATE], "商城id": [1], "市id": [10],
            "市名称": ["市A"], "费用类型": ["物流"], "费用计算方式": ["按比例"],
            "费率": [0.01], "区县费率映射": [""],
        })
...
class TestStage6Pricing:
    ...
    def test_direction_trial_higher_commission(self):
        ...
        products = pd.DataFrame({
            "商品id": [101], "商品编码": ["SKU001"], "商品名称": ["商品A"],
            "后台类目名称": ["类目1"], "非试验区域抽佣率": [0.02], "毛重": [1.0],
        })
        regions = pd.DataFrame({
            "日期": [TARGET_DATE], "试验区域id": [1001], "试验区域名称": ["区域A"],
            "市id": [10], "省id": [1], "区县id": [100],
            "是否试验区域": [1], "抽佣率": [0.05], "隐形物流费率": [0.01],
        })
    ...
    def test_direction_trial_zero_diff(self):
        products = pd.DataFrame({
            "商品id": [101], "商品编码": ["SKU001"], "商品名称": ["商品A"],
            "后台类目名称": ["类目1"], "非试验区域抽佣率": [0.03], "毛重": [1.0],
        })
        regions = pd.DataFrame({
            "日期": [TARGET_DATE], "试验区域id": [1001], "试验区域名称": ["区域A"],
            "市id": [10], "省id": [1], "区县id": [100],
            "是否试验区域": [1], "抽佣率": [0.03], "隐形物流费率": [0.01],
        })
        result = compute_pricing(products, regions)
...
class TestStage7Export:
    def test_output_columns_and_rename(self):
        df = pd.DataFrame({
            "试验区域id": [1001], "试验区域名称": ["区域A"],
            "商品编码": ["SKU001"], "商品名称": ["商品A"],
            "调价方向": ["涨价"], "调价幅度": [float("nan")],
            "设置状态": ["启用"], "固定抽佣比例": [0.02],
            "固定抽佣货值": [float("nan")], "商品id": [101],
            "后台类目名称": ["类目1"],
        })
    ...
    def test_empty_dataframe_writes_headers(self):
        df = pd.DataFrame(columns=[
            "试验区域id", "试验区域名称", "商品编码", "商品名称",
            "调价方向", "调价幅度", "设置状态", "固定抽佣比例",
            "固定抽佣货值", "商品id", "后台类目名称",
        ])
        with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as f:
            export_excel(df, f.name)
...
class TestFullPipeline:
    def test_full_pipeline(
        self, mock_conf_goods, mock_conf_trial_goods, mock_conf_county,
        mock_conf_trial_group, mock_conf_trial_commission,
        ...
        regions = associate_logistics_fee(regions, mock_conf_hidden_logistics, TARGET_DATE)
        result = compute_pricing(products, regions)
    ...
    def test_empty_products(
        self, mock_conf_trial_goods, mock_conf_county,
        mock_conf_trial_group, mock_conf_trial_commission,
        mock_conf_hidden_logistics,
    ):
        empty_goods = pd.DataFrame(columns=[
            "日期", "商城id", "商品id", "商品编码", "商品名称",
            "后台类目名称", "非试验区域抽佣率", "毛重",
        ])
        products = filter_trial_products(empty_goods, mock_conf_trial_goods, TARGET_DATE)
